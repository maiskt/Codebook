* Encoding: UTF-8.
*Calculating ppi_score by using PPI scorecard.
weight off.
...
recode dl22 (1=0) (2=4) into car.
recode dl23 (1=0) (2=7) into land.

compute ppi_score = sum(young, kid_edu, work, room, wall, tv, fan, cell_phone, car, land).
freq ppi_score.

***2.5 poverty line.
*Converting PPI scores to poverty likelihoods below each of the poverty lines by using lookup tables.
...
*mean of ppi_prob is 75.204267, finding the closest prob in ppi_score frequency tables, using the corresponding ppi_score 70 as the cupoff point.
compute POVERTY1 = 0.
if (ppi_score <= 70) POVERTY1 = 1.
...
*mean of ppi_prob is 21.923900, finding the closest prob in ppi_score frequency tables, using the corresponding ppi_score 44 as the cupoff point.
compute ppi_cutoff_125 = 0.
if (ppi_score <= 44 ) ppi_cutoff_125 = 1.

*derive poverty variable.
